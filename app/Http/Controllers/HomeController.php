    ...
    public function index()
    {
        ...
        if(!Auth::guard('orang_tua')->user()){
            if(Auth::user()->role->role_name == 'admin polsub'){
                ...
                $data = array(
                    'sp' => $sp,
                    'mahas_aktif' => User::where('status','aktif')->where('id_role',4)->where('id_prodi', Auth::user()->id_prodi)->count(),
                    'mahas_tidak' => User::where('status','tidak aktif')->where('id_role',4)->where('id_prodi', Auth::user()->id_prodi)->count()
                );
            }
        }else{
